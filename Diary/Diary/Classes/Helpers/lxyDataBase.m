...
- (void)createSandTimerTable
{
    ...
    //创建的sql语句
    NSString *sql = @"create table sandTimerTable (ID integer primary key not null,style text)";
...
- (void)insertToSandTimerTableWithOneSandTimerModel:(lxySandTimerModel *)sandTimerModel
{
    ...
    NSString *sql = [NSString stringWithFormat:@"insert into sandTimerTable(ID,style)values('%d' , '%@')", ID , style];
...
- (lxySandTimerModel *)searchOneSandTimerModel:(NSInteger)sandTimerModelID
{
    ...
    //准备sql语句
    NSString *sql = [NSString stringWithFormat:@"select ID, style from sandTimerTable where ID = ?"];
...
//字段（ID:ID  人名：name    记录的时间：time  记录的内容：content   记录的心情：mood  记录时的天气：weather）
...
- (void)createAlonePersonTable
{
    ...
    //创建的sql语句
    NSString *sql = @"create table alonePersonTable (ID integer primary key autoincrement not null,name text not null,time text not null,content text,mood text,weather text)";
//    NSString *sql = [NSString stringWithFormat:@"create table alonePersonTable (ID integer primary key not null, name text not null)"];
...
- (void)insertToAlonePersonTableWithOneAlonePersonModel:(lxyAlonePersonModel *)alonePersonModel
{
    ...
    NSString *mood = alonePersonModel.mood;
    NSString *weather = alonePersonModel.weather;
    
    NSString *sql = [NSString stringWithFormat:@"insert into alonePersonTable (ID, name, time, content, mood, weather)values('%d', '%@', '%@', '%@', '%@', '%@')", ID , name , time , content , mood , weather];
...
- (lxyAlonePersonModel *)searchOneAlonePersonModelByAlonePersonModelName:(NSString *)name
{
    ...
    //准备sql语句
    NSString *sql = [NSString stringWithFormat:@"select ID, name, time, content, mood, weather from alonePersonTable where name = ?"];
    ...
    if (result == SQLITE_OK) {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            //取mood
            NSString *mood = [NSString stringWithUTF8String:(const char *)sqlite3_column_text(stmt, 4)];
...
- (NSMutableArray *)searchAllDataFromalonePersonTable
{
    ...
    //准备sql语句
    NSString *sql = @"select * from alonePersonTable";
...
//字段（ID:ID  日记内容：content   日记时间：time）
...
- (void)createCommonDiaryTable
{
    ...
    //创建的sql语句
    NSString *sql = @"create table commonDiaryTable (ID integer primary key autoincrement not null,content text not null,time text not null)";
...
- (void)insertToCommonDiaryTableWithOneCommonDiaryModel:(lxyCommonDiaryModel *)commonDiaryModel
{
    ...
    NSString *sql = [NSString stringWithFormat:@"insert into commonDiaryTable (ID, time, content)values('%d', '%@', '%@')", ID , time , content];
...
- (lxyCommonDiaryModel *)searchOneCommonDiaryModelByTime:(NSString *)time
{
    ...
    //准备sql语句
    NSString *sql = [NSString stringWithFormat:@"select ID, content, time from commonDiaryTable where time = ?"];
